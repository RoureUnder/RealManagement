...
public class ProjectTaskHandler {
    ...
    @DeleteMapping("/delete")//删除对应的任务和文件
    public JSONObject deleteTask(@RequestBody ProjectTask projectTask){
        JSONObject jObject = new JSONObject();
        //删除本地文件project_file+project_file_review
        ProjectFileHandler projectFileHandler = new ProjectFileHandler();
        JSONObject project_file_delete = projectFileHandler.deleteByProjectFileNo(projectTask.getProjectFileNo());
        if(project_file_delete.get("Result").equals("success")){
            jObject.put("project_file_delete", project_file_delete);
            //删除对应文件审核记录
            ProjectFileReview projectFileReview = projectFileReviewRepository.findByTaskNo(projectTask.getNo());
            projectFileReviewRepository.delete(projectFileReview);
            //删除任务记录
            projectTaskRepository.delete(projectTask);
            jObject.put("project_file_review_delete", projectFileReview);
            jObject.put("ProjectTask", projectTask);
            jObject.put("Result", "success");
        }
        else{
            jObject.put("Message", "删除文件失败");
            jObject.put("Result", "error");
        }
        return jObject;
    }
    ...
    @PutMapping("/update")
    public JSONObject updateTask(@RequestBody ProjectTask projectTask){
        JSONObject jObject = new JSONObject();
        projectTask.setStatus(0);
        projectTask.setReason("");
        projectTask =  projectTaskRepository.save(projectTask);
